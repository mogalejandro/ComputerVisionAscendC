...
#include "Yolov3PostProcessNew.h"
...
namespace MxBase {
    Yolov3PostProcessNew &Yolov3PostProcessNew::operator=(const Yolov3PostProcessNew &other) {
        if (this == &other) {
            return *this;
        }
    ...
    APP_ERROR Yolov3PostProcessNew::Init(const std::map<std::string, std::string> &postConfig) {
        LogDebug << "Start to Init Yolov3PostProcessNew.";
        ...
        LogDebug << "End to Init Yolov3PostProcessNew.";
        return APP_ERR_OK;
    }

    APP_ERROR Yolov3PostProcessNew::DeInit() {
        return APP_ERR_OK;
    }

    bool Yolov3PostProcessNew::IsValidTensors(const std::vector <TensorBase> &tensors) const {
    ...
    void Yolov3PostProcessNew::ObjectDetectionOutput(const std::vector <TensorBase> &tensors,
                                                  std::vector <std::vector<ObjectInfo>> &objectInfos,
                                                  const std::vector <ResizedImageInfo> &resizedImageInfos) {
        LogDebug << "Yolov3PostProcessNew start to write results.";
        if (tensors.size() == 0) {
            return;
        }
        ...
        LogDebug << "Yolov3PostProcessNew write results successed.";
    }

    APP_ERROR Yolov3PostProcessNew::Process(const std::vector <TensorBase> &tensors,
                                         std::vector <std::vector<ObjectInfo>> &objectInfos,
                                         const std::vector <ResizedImageInfo> &resizedImageInfos,
                                         const std::map <std::string, std::shared_ptr<void>> &paramMap) {
        LogDebug << "Start to Process Yolov3PostProcessNew.";
        APP_ERROR ret = APP_ERR_OK;
        if (resizedImageInfos.size() == 0) {
            ret = APP_ERR_INPUT_NOT_MATCH;
            LogError << GetErrorInfo(ret) << "resizedImageInfos is not provided which is necessary for Yolov3PostProcessNew.";
            return ret;
        }
        auto inputs = tensors;
        ...
        LogDebug << "End to Process Yolov3PostProcessNew.";
        return APP_ERR_OK;
    }
 ...
 */
    void Yolov3PostProcessNew::CompareProb(int &classID, float &maxProb, float classProb, int classNum) {
 ...
 */
    void Yolov3PostProcessNew::SelectClassNCHW(std::shared_ptr<void> netout, NetInfo info,
    ...
    void Yolov3PostProcessNew::SelectClassNCHWC(std::shared_ptr<void> netout, NetInfo info,
 ...
 * @param layer  Yolo output layer
 */
    void Yolov3PostProcessNew::SelectClassNHWC(std::shared_ptr<void> netout, NetInfo info,
                                            std::vector <MxBase::ObjectInfo> &detBoxes, int stride, OutputLayer layer) {
        const int offsetY = 1;
        for (int j = 0; j < stride; ++j) {
            for (int k = 0; k < info.anchorDim; ++k) {
 ...
 * @param detBoxes  ObjectInfo vector where all ObjectInfoes's confidences are greater than threshold
 */
    void Yolov3PostProcessNew::GenerateBbox(std::vector <std::shared_ptr<void>> featLayerData,
    ...
    APP_ERROR Yolov3PostProcessNew::GetBiases(std::string &strBiases) {
        if (biasesNum_ <= 0) {
    ...
    extern "C" {
    std::shared_ptr <MxBase::Yolov3PostProcessNew> GetObjectInstance() {
        LogInfo << "Begin to get Yolov3PostProcessNew instance.";
        auto instance = std::make_shared<MxBase::Yolov3PostProcessNew>();
        LogInfo << "End to get Yolov3PostProcessNew instance.";
        return instance;
    }
    }
}